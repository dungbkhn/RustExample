...
//    while let sline = lines.next().await {
    ...
    /*	match original_stream {
     ...
     loop{
     	//if out==1 {println!();break;};
     	
	match lines.next().await {
	    Some(line) => {
	    		
	    		let line = line?;
			println!("server is receiving mes from {}:{}",name,line);
	    ...
	    _ => { println!("peer disconnected immediately;break loop here"); break; }
        }
    }
